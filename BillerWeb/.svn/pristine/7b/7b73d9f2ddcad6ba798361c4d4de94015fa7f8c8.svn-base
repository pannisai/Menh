...
public class BW5100Controller extends SearchPageController {

	public BW5100Controller() {
		super("Bank Service", "5100");
		// TODO Auto-generated constructor stub
	}

	private Logger log = Logger.getLogger(BW5100Controller.class);
	private static String SESS_BANK_SERVICE_FROM = "SESS_BANK_SERVICE_FROM";
	...
	protected void setupPageFromSession() {
		// TODO Auto-generated method stub
		try {
			BW5100Form bw5100 = (BW5100Form) getSession(SESS_BANK_SERVICE_FROM);

			if (bw5100 != null && bw5100.getBankCode() != null && bw5100.getBankService() != null && bw5100.getStatus() != null) {
				log.debug("InSetFormSession");
				try {
					lbBankService.setSelectedIndex(lbBankService.getIndexOfItem(bw5100.getBankService()));
					lbBankCode.setSelectedIndex(lbBankCode.getIndexOfItem(bw5100.getBankCode()));
					lbStatus.setSelectedIndex(lbStatus.getIndexOfItem(bw5100.getStatus()));
				...
				removeSession(SESS_BANK_SERVICE_FROM);
			}
	...
	private void setCurrentFormSession() {
		...
		try {
			BW5100Form bw5100Form = new BW5100Form();
			bw5100Form.setBankService(lbBankService.getSelectedItem());
			bw5100Form.setBankCode(lbBankCode.getSelectedItem());
			bw5100Form.setStatus(lbStatus.getSelectedItem());
			setSession(SESS_BANK_SERVICE_FROM, bw5100Form);
		} catch (Exception ex) {
	...
	private void search() {
		...
		ListModel<BankServiceSO> bankServiceSOListModel = null;
		...
		try {
			...
			BankServicebean criteria = new BankServicebean();
			...
			try {
				...
				criteria.setBANK_CODE(bankMasterBean.getBANK_CODE());
				status = lbStatus.getSelectedItem().getValue().toString();
				criteria.setACT_FLAG(status);
			} catch (NullPointerException npe) {

			}
			bankServiceSOs = bankServiceService.searchBankService(criteria, 0, 0, getUserInfo());
			bankServiceSOListModel = new SimpleListModel<BankServiceSO>(bankServiceSOs);

			setGridResultModel(bankServiceSOListModel);
		} catch (Exception ex) {
			Map<String, String> params = new HashMap<String, String>();
			params.put("Bank Service Id", bankServicebean.getBANK_SRVC_ID() + "");
			params.put("Bank Code", bankServicebean.getBANK_CODE() + "");
	...
	public void create() {
		...
		try {
			redirect(AppConfiguration.getValue(AppConfiguration.BW5110_REDIRECT_PATH));
		} catch (Exception ex) {
	...
	public void edit(ForwardEvent event) {
		...
		try {
			...
			Dialog.openBw5110MedalDialog(AppConfiguration.getValue(AppConfiguration.BW5100_REDIRECT_PATH), row.getValue().toString());
		} catch (Exception ex) {
