...
public class BW4100Controller extends SearchPageController {

	public BW4100Controller() {
		super("Bank Master","4100");
		// TODO Auto-generated constructor stub
	}

	private static String SESS_BANK_MASTER_FORM = "SESS_BANK_MASTER_FORM";

	private Logger log = Logger.getLogger(BW4100Controller.class);
	...
	protected void setupPageFromSession() {
		// TODO Auto-generated method stub
		try {
			BW4100Form bw4100 = (BW4100Form) getSession(SESS_BANK_MASTER_FORM);

			if (bw4100 != null && bw4100.getBankCode() != null && bw4100.getStatus() != null) {
				log.debug("InSetFormSession");
				try {
					lbBankCode.setSelectedIndex(lbBankCode.getIndexOfItem(bw4100.getBankCode()));
					lbStatus.setSelectedIndex(lbStatus.getIndexOfItem(bw4100.getStatus()));
				...
				removeSession(SESS_BANK_MASTER_FORM);
			}
	...
	private void setCurrentFormSession() {
		...
		try {
			BW4100Form bw4100Form = new BW4100Form();
			bw4100Form.setBankCode(lbBankCode.getSelectedItem());
			bw4100Form.setStatus(lbStatus.getSelectedItem());
			setSession(SESS_BANK_MASTER_FORM, bw4100Form);
		} catch (Exception ex) {
	...
	private void search() {
		...
		try {
			...
			bankMasterSOs = bankMasterService.searchBankMaster(criteria, 0, 0, getUserInfo());
			bankMasterSOListModel = new SimpleListModel<BankMasterSO>(bankMasterSOs);

			setGridResultModel(bankMasterSOListModel);
		} catch (Exception ex) {
	...
	public void create() {
		...
		try {
			redirect(AppConfiguration.getValue(AppConfiguration.BW4110_REDIRECT_PATH));
		} catch (Exception ex) {
	...
	public void edit(ForwardEvent event) {
		...
		try {
			...
			Dialog.openBw4110MedalDialog(AppConfiguration.getValue(AppConfiguration.BW4100_REDIRECT_PATH), row.getValue().toString());
		} catch (Exception ex) {
