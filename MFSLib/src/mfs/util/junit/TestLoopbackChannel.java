...
public class TestLoopbackChannel implements ISOFilter {
	public static void main(String[] args) {
		try {
			new TestLoopbackChannel().run();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void run() throws ISOException, IOException {
		...
		channel.addIncomingFilter(this);// (1)
	...
	public ISOMsg filter(ISOChannel channel, ISOMsg m, LogEvent evt) {// (2)
		try {
			m.setResponseMTI();
			m.set(39, "00");
