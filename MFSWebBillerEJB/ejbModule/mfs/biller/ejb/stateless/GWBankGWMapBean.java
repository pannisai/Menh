...
import mfs.biller.ejb.interfaces.GWBankGWMapBeanLocal;
...
@Stateless(name = "GWBankGWMapBean", mappedName = "mfs.biller.ejb.GWBankGWMapBean")
@CallByReference
public class GWBankGWMapBean implements GWBankGWMapBeanLocal,GWBankGWMapBeanRemote {
	
	private Logger log = Logger.getLogger("EJBGWBANKGWMap");
	private String page = "GWBankGWMapBean";
	@PersistenceContext(unitName = "mfs.db.MFSDAO")
	private EntityManager em;
	@SuppressWarnings("unchecked")
	public Collection<GWServiceMap> getGWBankGWMapAll(String DATA_MAP_ID,
	...
	public ObjMapGWxml findGWBankGWMap(int DATA_MAP_ID,  UserInfoBean user)
		...
		try {
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|findGWBankGWMap|Time:" + timer.getStartTime());
			...
			String sql = "SELECT t1.DATA_MAP_ID,t1.DATA_MAP_NAME,t2.XML_DATA_ID,t2.XML_DATA_TYPE,t2.XML_DATA_SRC,t2.XML_DATA_VRSN,t2.XML_DATA_DESC,t2.XML_DATA_STTS "
					+ "  FROM GW_BANK_GW_MAP t1 , GW_XML_DATA_SRC t2  "	
					+"  WHERE  t1.DATA_MAP_XML_SRC_ID = t2.XML_DATA_ID  AND  t1.DATA_MAP_ID = "+DATA_MAP_ID+" "	;

			log.info(user.getName() + "|" + page + "|findGWBankGWMap|sql:" + sql);

			Query query = em.createNativeQuery(sql);
			List<Object[]> list = query.getResultList();
			ObjMapGWxml ObjMapGWxml = new ObjMapGWxml(); 
			log.info(user.getName() + "|" + page + "|findGWBankGWMap|List.Size:" + list.size());
			...
			log.info(user.getName() + "|" + page + "|findGWBankGWMap|Time:" + timer.getStopTime());
				return ObjMapGWxml;
				
		} catch (Exception e) {
			log.error(ExceptionUtils.getStackTrace(e));
			log.info(user.getName() + "|" + page + "|findGWBankGWMap|Exception:" + e);
			log.info(user.getName() + "|" + page + "|findGWBankGWMap|Exception:" + e.getMessage());
			throw e;
		}
	...
	public int insertGWBankGWMap(ObjMapGWxml bean, UserInfoBean user)throws IsExistException, Exception{
		try{
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|ObjMapGWxml|Time|" + timer.getStartTime());
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|ObjMapGWxml|Param|" + bean.toString());
			...
			Query query1 = em.createNativeQuery("SELECT SEQ_GW_BANK_GW_MAP.nextval from DUAL");
			BigDecimal result1 = (BigDecimal)query1.getSingleResult();
			int DATA_MAP_ID = result1.intValue();
			
			
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|DATA_MAP_ID:" + DATA_MAP_ID);
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|XML_DATA_ID:" + XML_DATA_ID);
			
			String sql = "INSERT INTO GW_BANK_GW_MAP(DATA_MAP_ID, DATA_MAP_NAME, DATA_MAP_XML_SRC_ID )"
					   + "VALUES(?, ?, ?)";
			int i = 0;
			...
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|sql"+sql);
			...
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|sql"+sql1);
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|Success");
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|ObjMapGWxml|Time|" + timer.getStopTime());
			return DATA_MAP_ID;
			
		}catch(Exception e){
			em.getTransaction().rollback();
			log.info(user.getName() + "|" + page + "|insertGWBankGWMap|Exception:" + e.getMessage());
			throw e;
		}
	...
	public void updateGWBankGWMap(ObjMapGWxml bean, UserInfoBean user)throws IsExistException, Exception{
		try{
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Time|" + timer.getStartTime());
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Param|" + bean.toString());

			Query query1 = em.createNativeQuery("SELECT  T1.XML_DATA_ID  from GW_XML_DATA_SRC T1 join GW_BANK_GW_MAP T2  on T1.XML_DATA_ID = T2.DATA_MAP_XML_SRC_ID  WHERE  T2.DATA_MAP_ID = "+bean.getDATA_MAP_ID()+"");
			BigDecimal result = (BigDecimal)query1.getSingleResult();
			int XML_DATA_ID = result.intValue();
			StringBuffer sb = new StringBuffer();
			sb.append("UPDATE GW_BANK_GW_MAP ")
			...
			query.executeUpdate();			
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Success");
			...
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Success");
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Time|" + timer.getStopTime());
	
		}catch(Exception e){
			em.getTransaction().rollback();
			log.info(user.getName() + "|" + page + "|updateGWBankGWMap|Exception:" + e.getMessage());
			throw e;
		}
