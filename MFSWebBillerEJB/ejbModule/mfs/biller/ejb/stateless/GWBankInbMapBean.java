...
import mfs.biller.ejb.interfaces.GWBankInbMapBeanLocal;
...
@Stateless(name = "GWBankInbMapBean", mappedName = "mfs.biller.ejb.GWBankInbMapBean")
@CallByReference
public class GWBankInbMapBean implements GWBankInbMapBeanRemote,
GWBankInbMapBeanLocal {
	
	private Logger log = Logger.getLogger("EJBBANKINBMAP");
	private String page = "GWBankInbMapBean";
	...
	public Collection<GWBankInbMap> getGWInboundMapAll(String DATA_MAP_ID,
		...
		try {
			...
			Collection<GWBankInbMap> colreturn = new Vector();

			Query query = em.createNativeQuery(sql, GWBankInbMap.class);
			...
			for (int i = 0; i < list.size(); i++) {
				Object o = list.get(i);
				GWBankInbMap mastTrnsPrt = (GWBankInbMap) o;
	...
	public ObjMapGWxml findGWBankInbMap(int DATA_MAP_ID,  UserInfoBean user)
		...
		try {
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|findGWBankInbMap|Time:" + timer.getStartTime());
			String sql = "SELECT t1.DATA_MAP_ID,t1.DATA_MAP_NAME,t2.XML_DATA_ID,t2.XML_DATA_TYPE,t2.XML_DATA_SRC,t2.XML_DATA_VRSN,t2.XML_DATA_DESC,t2.XML_DATA_STTS "
					+ "  FROM GW_BANK_INB_MAP t1 , GW_XML_DATA_SRC t2  "	
					+"  WHERE  t1.DATA_MAP_XML_SRC_ID = t2.XML_DATA_ID  AND  t1.DATA_MAP_ID = "+DATA_MAP_ID+" "	;

			log.info(user.getName() + "|" + page + "|findGWBankInbMap|sql:" + sql);
			Query query = em.createNativeQuery(sql);
			List<Object[]> list = query.getResultList();
			ObjMapGWxml ObjMapGWxml = new ObjMapGWxml(); 
			log.info(user.getName() + "|" + page + "|findGWBankInbMap|List.Size:" + list.size());
			...
			log.info(user.getName() + "|" + page + "|findGWBankInbMap|Time:" + timer.getStopTime());
			return ObjMapGWxml;
			
			
		} catch (Exception e) {
			log.error(ExceptionUtils.getStackTrace(e));
			log.info(user.getName() + "|" + page + "|findGWBankInbMap|Exception:" + e);
			log.info(user.getName() + "|" + page + "|findGWBankInbMap|Exception:" + e.getMessage());
			throw e;
		}
	...
	public int insertGWBankInbMap(ObjMapGWxml bean, UserInfoBean user)throws IsExistException, Exception{
		try{
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|Time|" + timer.getStartTime());
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|Param|" + bean.toString());
			...
			Query query1 = em.createNativeQuery("SELECT SEQ_GW_BANK_INB_MAP.nextval from DUAL");
			BigDecimal result1 = (BigDecimal)query1.getSingleResult();
			int DATA_MAP_ID = result1.intValue();
			
			
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|DATA_MAP_ID:" + DATA_MAP_ID);
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|XML_DATA_ID:" + XML_DATA_ID);
			
			String sql = "INSERT INTO GW_BANK_INB_MAP(DATA_MAP_ID, DATA_MAP_NAME, DATA_MAP_XML_SRC_ID )"
					   + "VALUES(?, ?, ?)";
			int i = 0;
			...
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|sql"+sql);
			...
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|sql"+sql1);
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|Success");
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|Time|" + timer.getStopTime());
			return DATA_MAP_ID;
			
		}catch(Exception e){
			em.getTransaction().rollback();
			log.info(user.getName() + "|" + page + "|insertGWBankInbMap|Exception:" + e.getMessage());
			throw e;
		}
	...
	public void updateGWBankInbMap(ObjMapGWxml bean, UserInfoBean user)throws IsExistException, Exception{
		try{
			...
			Timer timer = new Timer("-");
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Time|" +timer.getStartTime());
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Param|" + bean.toString());


			Query query1 = em.createNativeQuery("SELECT  T1.XML_DATA_ID  from GW_XML_DATA_SRC T1 join GW_BANK_INB_MAP T2  on T1.XML_DATA_ID = T2.DATA_MAP_XML_SRC_ID  WHERE  T2.DATA_MAP_ID = "+bean.getDATA_MAP_ID()+"");
			BigDecimal result = (BigDecimal)query1.getSingleResult();
			int XML_DATA_ID = result.intValue();
			StringBuffer sb = new StringBuffer();
			sb.append("UPDATE GW_BANK_INB_MAP ")
			...
			query.executeUpdate();			
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Success");
			...
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Success");
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Time|" +timer.getStopTime());
	
		}catch(Exception e){
			em.getTransaction().rollback();
			log.info(user.getName() + "|" + page + "|updateGWBankInbMap|Exception:" + e.getMessage());
			throw e;
		}
	}
